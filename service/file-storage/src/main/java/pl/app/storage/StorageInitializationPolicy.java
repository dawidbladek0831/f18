...
class StorageInitializationPolicy {
    private final Logger logger = LoggerFactory.getLogger(StorageInitializationPolicy.class);
    private final StorageService storageService;

    @KafkaListener(
            id = "container-created-event-listener--storage",
    ...
    public void containerCreated(ConsumerRecord<ObjectId, ContainerEvent.ContainerCreated> record) {
        ...
        storageService.init(event.getContainerId()).subscribe();
    }
}
